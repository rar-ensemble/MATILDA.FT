...
Dynamic::Dynamic(istringstream &iss) : ExtraForce(iss)
{
    ...
    RN_ARRAY.resize(n_donors * n_acceptors);
    ...
    for (int i = 0; i < n_donors; ++i){
        ...
        for (int j = 0; j < ACCEPTORS.size(); ++j){
            RN_ARRAY[i * n_acceptors + j] = ACCEPTORS[j];
        }
    }
            ...
            if (rnd == 0){
                ...
                if (n_free > 0){
                    d_make_bonds<<<GRID, threads>>>(d_x,d_f,
                        ...
                        k_spring, e_bond, r0, r_n, qind, d_L, d_Lh, Dim, d_charges);
                    ...
                    for (int i = 0; i < group->nsites; ++i){
                        if (AD[i] == 1 && BONDS[2*i] == 1){
                            BONDED[n_bonded++] = i;
                        }
                        else if (AD[i] == 1 && BONDS[2*i] == 0){
                            FREE[n_free++] = i;
                        }
                    }
                ...
                if (n_bonded > 0 && ((double) rand() / (RAND_MAX)) > float(n_free)/float(n_donors)){
    ...
    float* d_charges)
{
    ...
    d_states[ind] = l_state;
    lnid = d_RN_ARRAY[list_ind * n_acceptors + r%c];
...
void Dynamic::WriteBonds(void)
{
    ...
    bond_file << global_step << " " << float(n_bonded)/min(n_donors,n_acceptors) << endl;
    // bond_file << "TIMESTEP: " << global_step << " " << n_bonded << " " << n_free << " " << n_free + n_bonded << endl;
    // for (int j = 0; j < group->nsites; ++j)
    // {
    //     if (BONDS[2 * j + 1] != -1 && AD[j] == 1)
    //     {
