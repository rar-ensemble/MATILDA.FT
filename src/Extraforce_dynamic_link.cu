...
#include "Extraforce_dynamic_link.h"
...
Dynamic_Link::~Dynamic_Link() { return; }

Dynamic_Link::Dynamic_Link(istringstream &iss) : ExtraForce(iss)
{



    readRequiredParameter(iss, link);
    cout << "link: " << link << endl;
    ...
    cout << "Dynamic_Link bonds active!" << endl;
...
void Dynamic_Link::IncreaseCapacity(){
...
void Dynamic_Link::UpdateBonders(){
...
void Dynamic_Link::AddExtraForce()
{   
    ...
    {
        Dynamic_Link::WriteBonds();
    }

    if (step % traj_freq == 0 && traj_freq > 0)
    {
        Dynamic_Link::write_resume_files();
    }
}


/*
Updates forces acting on particles due to Dynamic_Link bonds
*/
...
__global__ void d_make_bonds_linked(
    const float *x,
    float* f,
    thrust::device_ptr<int> d_BONDS,
    ...
    int D)
{
    ...
    int lnid = d_LINKED_BONDS[list_ind];
...
void Dynamic_Link::WriteBonds(void)
{
...
void Dynamic_Link::UpdateVirial(void){
...
void Dynamic_Link::write_resume_files(){
