...
__global__ void d_prepareElectricField(cufftComplex* d_cpxx, cufftComplex* d_cpxy,cufftComplex* d_cpxz, cufftComplex* d_ep,
	...
	float exp_k = 1;//exp(-k2 * length_scale * length_scale);
...
__global__ void d_prepareForceKSpace(cufftComplex* fk, cufftComplex* rhok,
	cufftComplex* rhofkx, cufftComplex* rhofky, cufftComplex* rhofkz, const int Dim, const int M) {
	...
	rhofkx[ind].x = (fk[ind * Dim + dir].x * rhok[ind].x
		- fk[ind * Dim + dir].y * rhok[ind].y) / float(M);

	rhofkx[ind].y = (fk[ind * Dim + dir].x * rhok[ind].y 
		+ fk[ind * Dim + dir].y * rhok[ind].x) / float(M);

	dir = 1;
	rhofky[ind].x = (fk[ind * Dim + dir].x * rhok[ind].x
		- fk[ind * Dim + dir].y * rhok[ind].y) / float(M);

	rhofky[ind].y = (fk[ind * Dim + dir].x * rhok[ind].y 
		+ fk[ind * Dim + dir].y * rhok[ind].x) / float(M);

	dir = 2;

	rhofkz[ind].x = (fk[ind * Dim + dir].x * rhok[ind].x
		- fk[ind * Dim + dir].y * rhok[ind].y) / float(M);

	rhofkz[ind].y = (fk[ind * Dim + dir].x * rhok[ind].y 
		+ fk[ind * Dim + dir].y * rhok[ind].x) / float(M);
}
...
__global__ void d_accumulateGridForcex2(cufftComplex* frho,
	float* d_t_rho, float* d_t_fdir, const int type, const int M) {
	...
	d_t_fdir[type * M + ind] = 2.0f * (d_t_fdir[type * M + ind] +
		d_t_rho[type * M + ind] * frho[ind].x);
