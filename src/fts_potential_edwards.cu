...
PotentialEdwards::PotentialEdwards(std::istringstream& iss, FTS_Box* p_box) : FTS_Potential(iss, p_box) {
    ...
    while (iss.tellg() != -1 ) {
        ...
        if ( s1 == "initialize" ) {
            ...
            iss >> s1;
            if ( s1 != "zeromean" && s1 != "zeroMean" ) { die("Invalid modify option on Edwards potential"); }
            zeroMean = true;
        }
    }// optional arguments
...
void PotentialEdwards::correctFields() {
    if ( updateScheme != "EMPC" ) {
        return;
    }
...
void PotentialEdwards::storePredictorData() {
    d_wplo = d_wpl;
    d_dHdwplo = d_dHdw;
}
...
void PotentialEdwards::writeFields(int potInd ) { 
    char nm[30];
    sprintf(nm, "wpl_Edwards%d.dat", potInd);
...
std::complex<double> PotentialEdwards::calcHamiltonian() {
...
void PotentialEdwards::initLinearCoeffs() {
    // Akpl = C / kappaN
    thrust::fill(d_Akpl.begin(), d_Akpl.end(), 1.0/B);
}

PotentialEdwards::~PotentialEdwards() {}