...
__global__ void d_accumulateGridForce(cufftComplex*, float*, float*,
    const int, const int);
__global__ void d_accumulateGridForcex2(cufftComplex*, float*, float*,
    const int, const int);
...
void Potential::CalcForces() {
    ...
    if (SAME_TYPE != 1){
        ...
        d_prepareForceKSpace<<<M_Grid, M_Block>>>(this->d_f_k, d_calculated_rho_all + M * type2, d_cpxx, d_cpxy, d_cpxz,Dim, M);
            ...
            d_accumulateGridForce<<<M_Grid, M_Block, 0, 0>>>(d_cpxx,
    ...
    if(SAME_TYPE == 1){
        d_accumulateGridForcex2<<<M_Grid, M_Block, 0, 0>>>(d_cpxx,
...
//                 d_calculated_rho_all[type2], d_cpx1, j, Dim, M);
...
//             // Accumulate the forces on type 1
//             if (j == 0)
//                 d_accumulateGridForce<<<M_Grid, M_Block, 0, stream1>>>(d_cpx1,
//                 d_all_rho, d_all_fx, type1, M);

//             if (j == 1)
//                 d_accumulateGridForce<<<M_Grid, M_Block, 0, stream2>>>(d_cpx1,
//                 d_all_rho, d_all_fy, type1, M);

//             if (j == 2)
//                 d_accumulateGridForce<<<M_Grid, M_Block, 0, stream3>>>(d_cpx1,
//                 d_all_rho, d_all_fz, type1, M);
...
//         // d_cpx1 = d_cpx2 * d_f_k
//         d_prepareForceKSpace<<<M_Grid, M_Block>>>(this->d_f_k,
//             d_calculated_rho_all[type1], d_cpx1, j, Dim, M);
...
//         if (j == 0){
//             if(SAME_TYPE == 1){
//                 d_accumulateGridForcex2<<<M_Grid, M_Block, 0, stream1>>>(d_cpx1,
//                     d_all_rho, d_all_fx, type2, M);
//             }
//             else{
//                 d_accumulateGridForce<<<M_Grid, M_Block, 0, stream1>>>(d_cpx1,
...
//         if (j == 1){
//                 if(SAME_TYPE == 1){
//                     d_accumulateGridForcex2<<<M_Grid, M_Block, 0, stream2>>>(d_cpx1,
//                         d_all_rho, d_all_fy, type2, M);
//                 }
//                 else{
//                     d_accumulateGridForce<<<M_Grid, M_Block, 0, stream2>>>(d_cpx1,
...
//         if (j == 2){
//                 if(SAME_TYPE == 1){
//                     d_accumulateGridForcex2<<<M_Grid, M_Block, 0, stream3>>>(d_cpx1,
//                         d_all_rho, d_all_fz, type2, M);
//                 }
//                 else{
//                     d_accumulateGridForce<<<M_Grid, M_Block, 0, stream3>>>(d_cpx1,
