...
// Defines the spline weights for the (pmeorder+1) nearby grid points
// See Appendix E in Deserno, Holm, J. Chem. Phys. V109 p7678 (1998)
...
// H: grid spacing in this direction
// W [pmeorder+1] weights for each grid point
// pmeorder: order of the PM spline.
__device__ void spline_get_weights(float dx, float H, float* W, 
    int pmeorder ) {

    // Define the distance so sx \in [-0.5, 0.5]
    float sx = dx / H;
