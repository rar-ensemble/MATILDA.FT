...
__global__ void d_multiplyCuDoubleComplexByDouble(
    ...
    const double val,        // value of inp to be multiplied by
    const int N
    ) {
    const int id = blockIdx.x * blockDim.x + threadIdx.x;
    if (id >= N)
...
// nn[Dim] in [ 0 , Nx[Dim] )
__device__ void d_unstack(int id, int* nn, const int* Nx, const int Dim ) {
...
__global__ void d_scale_by_minusk2(
    cuDoubleComplex *k2w,  // [M] resulting field after scaling by k^2
    const cuDoubleComplex *w,   // [M] input field
    const float* L,        // [Dim] box dimensions
    const int* Nx,         // [Dim] grid dimensions
    const int Dim,
    const int M
) {
    ...
    k2 = d_get_k(idx, kvec, L, Nx, Dim);

    k2w[idx].x = -k2 * w[idx].x;
    k2w[idx].y = -k2 * w[idx].y;
